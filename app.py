...
# --- MÓDULO DE OCR CON DEPURACIÓN AVANZADA ---
def get_matches_from_image_with_ocr(image_bytes, api_key):
    log = ["Iniciando proceso de OCR..."]
    try:
        log.append("Inicializando cliente de OpenAI...")
        client = OpenAI(api_key=api_key)
        
        log.append("Codificando imagen a base64...")
        base64_image = base64.b64encode(image_bytes).decode('utf-8')
        
        prompt_text = "Analiza la imagen de la quiniela Progol. Extrae los 21 partidos (14 regulares + 7 revancha). Devuelve un único array JSON con 21 objetos, cada uno con claves 'home' y 'away'. Limpia los nombres de los equipos. Si es ilegible, devuelve un array JSON vacío: []. Tu respuesta DEBE ser únicamente el JSON."
        log.append("Prompt creado. Enviando a la API de OpenAI...")
        ...
        log.append("Respuesta recibida de la API.")
        
        raw_response = response.choices[0].message.content.strip()
        log.append("Extrayendo contenido de la respuesta.")
        
        start, end = raw_response.find('['), raw_response.rfind(']')
        if start != -1 and end != -1 and end > start:
            json_string = raw_response[start:end+1]
            log.append("Se encontró un posible array JSON. Intentando decodificar...")
            try:
                parsed_json = json.loads(json_string)
                log.append("¡Decodificación de JSON exitosa!")
                return parsed_json, log, prompt_text, raw_response
            except json.JSONDecodeError as e:
                log.append(f"ERROR: El texto extraído no es un JSON válido. Error: {e}")
                return None, log, prompt_text, raw_response
        else:
            log.append("ERROR: No se encontró un array JSON en la respuesta.")
            return None, log, prompt_text, raw_response
            
    except Exception as e:
        log.append(f"EXCEPCIÓN CRÍTICA: {e}")
        st.error(f"Error con la API de OpenAI: {e}")
        return None, log, prompt_text, "No se pudo obtener respuesta debido a una excepción."
...
with st.sidebar.expander("🤖 Modo Automático (Experimental)"):
    ...
    debug_mode = st.checkbox("Activar modo de depuración", key="debug_check")
        ...
        if st.button("2. Leer Partidos con IA (OCR)", key="ocr_button"):
            ...
            if not api_key: st.error("Configura tu 'OPENAI_API_KEY' en los secretos de Streamlit Cloud.")
                ...
                with st.spinner("La IA está analizando la imagen..."):
                    extracted_matches, log, prompt, response = get_matches_from_image_with_ocr(image_bytes, api_key)
                st.session_state.ocr_debug_info = {"log": log, "prompt": prompt, "response": response}
...
if debug_mode and st.session_state.get('ocr_debug_info'):
    with st.expander("🐞 Información de Depuración de OCR"):
        info = st.session_state.ocr_debug_info
        st.write("**Registro de Eventos (Log):**"); st.code("\n".join(info.get("log", [])))
        if info.get("prompt"): st.write("**Prompt Enviado:**"); st.text(info["prompt"])
        if info.get("response"): st.write("**Respuesta Cruda Recibida:**"); st.text(info["response"])
    st.session_state.ocr_debug_info = None
...
if isinstance(st.session_state.get('final_df'), pd.DataFrame) and not st.session_state.final_df.empty:
    ...
    # ... (código de los sliders de la sidebar) ...
    ...
    if st.button("🔥 Iniciar Optimización Avanzada", type="primary"):
        with st.spinner("Aplicando reglas de modelado heurístico..."):
            ...
            # Validar que las columnas de probabilidad existen antes de usarlas
            if not all(c in df_modelado.columns for c in ['p_L', 'p_E', 'p_V']):
                st.error("El DataFrame final no contiene las columnas de probabilidad necesarias (p_L, p_E, p_V).")
                st.stop()
